...
export interface Journey {
  ...
  createdAt: any; // Firestore Timestamp or mock
}
...
const App: React.FC = () => {
  ...
  // =========================
  // Firestore: Load Journeys
  // =========================
  useEffect(() => {
  const fetchJourneys = async () => {
    console.log("Retrieving journeys...");
    try {
      const q = query(collection(db, "journeys"), orderBy("createdAt", "desc"));
      const querySnapshot = await getDocs(q);
      console.log("Journeys retrieved:", querySnapshot.size);
      const fetchedJourneys = querySnapshot.docs.map(doc => {
        const data = doc.data();
        console.log("Doc from Firestore:", doc.id, data);
        return { id: doc.id, ...data } as Journey;
      });
  ...
  const handleGenerateAssignmentJourney = useCallback(async (text: string) => {
    ...
    try {
      ...
      console.error("Error during assignment journey generation:", err);
      setError(
        err instanceof Error ? err.message : 'An unexpected error occurred.'
      );
  ...
  const handleNextModule = useCallback(() => {
    ...
    if (currentModuleIndex < modules.length - 1) {
      ...
      setDuckStats((prev) => ({
        ...prev,
        coins: prev.coins + JOURNEY_REWARD_COINS,
      }));
  ...
  const handleIncorrectAnswer = useCallback(
    async (module: Module, questionIndex: number) => {
  ...
  const handleTimerComplete = useCallback(
    () => {
      if (appState === 'journey') {
        setDuckStats((prev) => ({
          ...prev,
          coins: prev.coins + FOCUS_SESSION_REWARD_COINS,
        }));
  ...
  const handleSaveJourney = async () => {
    ...
    try {
      ...
      console.log("about to call addDoc with journey:", newJourneyData);
  
      const docRef = await addDoc(journeyCollection, newJourneyData);
      console.log("Journey saved in Firestore. Doc id:", docRef.id);
  
      const newJourneyForState: Journey = {
        ...newJourneyData,
        id: docRef.id,
      ...
      console.log("Journey added to state.");
      ...
      setNotification(
        `Failed to save journey. See console for details. Error: ${
  ...
  const generatePdf = async (answers: { [key: string]: string }) => {
    ...
    let html =
      '<h1 style="font-size: 28px; font-weight: 700; color: #6D28D9; margin-bottom: 24px;">Assignment Answers</h1>';

    for (const question in answers) {
      html += `
        <div style="margin-bottom: 20px; page-break-inside: avoid;">
          <h2 style="font-size: 18px; font-weight: 700; margin-bottom: 8px;">${question}</h2>
          <p style="font-size: 14px; color: #6B7280; white-space: pre-wrap; line-height: 1.6;">${
            answers[question]
          }</p>
        </div>
      `;
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-4 relative">
          ...
          <main className="flex-1 flex items-center justify-center pt-20 pb-4 px-4 w-full">
            {appState === 'journey' ? (
              ...
              <GameModule
                onGameEnd={handleTimerComplete}
                stats={duckStats}
                // ✅ FIX: use the correct handler name
                onUpdateStats={handleUpdateDuckStats}
              />
            )}
          </main>
        </div>
    ...
    switch (appState) {
      case 'dashboard': {
        const enrichedJourneys = journeys.map((j) => ({
          ...j,
          highestModuleIndex: j.highestModuleIndex ?? j.currentModuleIndex,
        }));
        ...
        return (
          <Dashboard
            ...
            onLoadJourney={(modules, index, title, highestIndex) =>
              handleLoadJourney(modules, index, title, highestIndex)
            }
      ...
      case 'loading':
        return (
          <LoadingGame
            key="loading"
            message="AI is building your personalized journey..."
          />
        );
